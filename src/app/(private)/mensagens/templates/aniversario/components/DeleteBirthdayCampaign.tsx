...
import { deleteBirthdayMessage } from '@/services/message/birthdayMessage'
import { Trash2 } from 'lucide-react'
import { useRouter } from 'next/navigation'
import { useCallback, useRef, useState } from 'react'

type DeleteBirthdayCampaignProps = {
  id: string
  campaignName: string
}

export default function DeleteBirthdayCampaign({
  id,
  campaignName,
}: DeleteBirthdayCampaignProps) {
  ...
  const handleDelete = useCallback(async () => {
    setIsDeleting(true)
    try {
      const res = await deleteBirthdayMessage(id)
