...
const LIST_BASE = '/mensagens/templates/depois-do-agendamento'
...
export default async function EditAfterScheduleCampaignPage({
  params,
  searchParams,
}: PageProps) {
  ...
  return (
    <div className="w-full max-w-screen-lg space-y-6">
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-purple-50">
            <CalendarCheck className="h-5 w-5 text-main" />
          </div>
          <div>
            <h1 className="text-2xl font-semibold text-main">
              {messageData?.name ?? 'Mensagem após agendamento'}
            </h1>
      ...
      {activeTab === 'configuracao' ? (
        <AfterScheduleForm defaultValues={messageData} />
      ) : sentLogsProps ? (
