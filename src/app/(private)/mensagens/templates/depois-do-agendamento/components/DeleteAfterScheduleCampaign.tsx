...
import { deleteAfterScheduleMessage } from '@/services/message/afterScheduleMessage'
import { Trash2 } from 'lucide-react'
import { useRouter } from 'next/navigation'
import { useCallback, useRef, useState } from 'react'

type DeleteAfterScheduleCampaignProps = {
  id: string
  campaignName: string
}

export default function DeleteAfterScheduleCampaign({
  id,
  campaignName,
}: DeleteAfterScheduleCampaignProps) {
  ...
  const handleDelete = useCallback(async () => {
    setIsDeleting(true)
    try {
      const res = await deleteAfterScheduleMessage(id)
