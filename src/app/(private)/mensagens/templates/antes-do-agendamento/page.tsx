...
export default async function BeforeScheduleTemplatePage({
  searchParams,
}: PageProps) {
  const activeTab = searchParams.aba ?? 'configuracao'

  const sentData = await listBeforeScheduleMessages().then((res) =>
    ...
    <div className="w-full max-w-screen-lg space-y-6">
      ...
      <div className="flex gap-1 border-b border-slate-200">
        ...
        >
          ...
          Mensagens Enviadas
          {(sentData?.total ?? 0) > 0 && (
            <span className="rounded-full bg-main px-2 py-0.5 text-[10px] font-semibold text-white">
              {sentData?.total}
            </span>
          )}
        </Link>
      ...
      {activeTab === 'configuracao' ? (
        ...
        <SentMessagesList data={sentData} />
      )}
    </div>
  )
}
