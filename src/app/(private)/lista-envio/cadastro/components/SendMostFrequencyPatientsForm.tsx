...
const KIND = 'send_most_frequency_patients' as const

export type SendMostFrequencyPatientsFormProps =
  | {
      mode: 'create'
    }
  | {
...
function isEditProps(
  props: SendMostFrequencyPatientsFormProps,
): props is Extract<SendMostFrequencyPatientsFormProps, { mode: 'edit' }> {
  return props.mode === 'edit'
}

export default function SendMostFrequencyPatientsForm(
  props: SendMostFrequencyPatientsFormProps,
) {
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:items-end sm:gap-4">
        <Input.Root className="w-full min-w-0">
          ...
          <Input.Field
            ...
            placeholder="Ex.: Pacientes mais frequentes"
            autoComplete="off"
            disabled={submitting}
            error={!!nameError}
          />
